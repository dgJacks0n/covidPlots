...
---
abstract: |
  COVID-19 case data was downloaded from the Johns Hopkins Dataset.
  Worldwide there were `r format(sum(last$confirmed), big.mark = ",")` cases and `r format(sum(last$deaths), big.mark = ",")` deaths as of `r max(last$date)`.
  In `r flagCountry` there were `r format(max(flagCases$confirmed), big.mark=",")` cases and `r format(max(flagCases$deaths), big.mark = ",")` deaths as of `r max(flagCases$date)`.
---

...
lastStateData <- getLastStateData()

lastCountyData <- getLastCountyData()
...
newCasesState <- stateData %>%
	...
	mutate(rmean_new_cases = 0.1 + rollmean(new_cases, 5, fill = NA, align = "right")) %>%
	ungroup() %>%
	filter(!is.na(rmean_new_cases)) %>%
	highlight_key(~state, group = "byState") %>%
	ggplot(aes(x = date, y = rmean_new_cases)) +
	geom_line(aes(group = state)) +
	labs(y = "New Cases (5 day average)", x = "Date",

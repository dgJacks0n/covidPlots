#' Plot windowed doubling time in days for cases or deaths by state
...
plotDoublingTime <- function(eData = getUsStateCases(), event = c("cases", "deaths"), myWindow = 7,
	...
	dt_col <- paste(myEvent, "doubling_time", sep = "_")
	...
	sPlot <- ggplot2::ggplot(growthRateData, 
		...
		ggplot2::labs(x = "Date", y = "Doubling Time", title = dt_colLabel, 
...
# doublingRate: calculate doubling rate over window
...
fit_series <- function(vals) {
	# calculate doubling time within window
